...
*@param Tournament $tournament
	...
	public function energyRule_PanelSteepness($line, $round) {
		...
		$roomPotential = $line->getLevel() - $round*2;
		...
		foreach($line->getAdjudicators() as $adjudicator_add){
			$total_rating_for_panel += $adjudicator_add;
			$n++;
		}
		...
		($average_rating_for_panel - $mean_for_round) / $standard_deviation_for_round = $comparison_factor;
