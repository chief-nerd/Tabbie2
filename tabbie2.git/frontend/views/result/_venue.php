...
if ($model->result instanceof Result) {
	...
	$popcontent = "<table width='100%'>"
		...
		. "</tr><tr>"
		. "<td>"
		. (($model->og_team->speakerA) ? $model->og_team->speakerA->name : \common\models\User::NONE) . ": " . $result->getSpeakerSpeaks(Team::getPos(Team::OG), Team::POS_A) . "<br/>"
		...
		. "<td>"
		. (($model->oo_team->speakerA) ? $model->oo_team->speakerA->name : \common\models\User::NONE) . ": " . $result->getSpeakerSpeaks(Team::getPos(Team::OO), Team::POS_A) . "<br/>"
		...
		. "</tr><tr>"
		. "<td>"
		. (($model->cg_team->speakerA) ? $model->cg_team->speakerA->name : \common\models\User::NONE) . ": " . $result->getSpeakerSpeaks(Team::getPos(Team::CG), Team::POS_A) . "<br/>"
		...
		. "<td>"
		. (($model->co_team->speakerA) ? $model->co_team->speakerA->name : \common\models\User::NONE) . ": " . $result->getSpeakerSpeaks(Team::getPos(Team::CO), Team::POS_A) . "<br/>"
		. (($model->co_team->speakerB) ? $model->co_team->speakerB->name : \common\models\User::NONE) . ": " . $result->getSpeakerSpeaks(Team::getPos(Team::CO), Team::POS_B) . "</td>"
		. "</tr>"
		. "</table>";
} else
