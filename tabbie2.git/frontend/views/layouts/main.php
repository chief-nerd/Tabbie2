    ...
    if ($this->context->hasMethod("_getContext")) {
        ...
        if ($tournament instanceof \common\models\Tournament) {
            ...
            if ($tournament->isTabMaster(Yii::$app->user->id) || $tournament->isConvenor(Yii::$app->user->id) || $tournament->isCA(Yii::$app->user->id)) {
                ...
                if ($tournament->isLanguageOfficer(Yii::$app->user->id)) {
        ...
        <div class="container">
            <div class="dropup pull-left">
                <a href="#" data-toggle="dropdown"
                   class="dropdown-toggle"><?= common\models\Language::getLabel(Yii::$app->language) ?> <b
                            class="caret"></b></a>
                <?= \frontend\components\LanguageSwitcher::widget(["id" => "lang"]) ?>
            </div>

            <p class="pull-left">
                <?= "&nbsp;&nbsp;" ?>
                <?= Yii::$app->params["appName"] ?> &copy; <?= date('Y') ?>
                -
                <small><?= Yii::t("app", "made with secret alien technology") ?></small>
            </p>

            <p class="pull-right">
                <?= HTML::a(Yii::t("app", "Report a Bug"), Yii::$app->params["bugURL"], ["target" => "_blank"]) ?>
                <?= " | " ?>
                <?= HTML::a(Yii::t("app", "Privacy Policy"), Yii::$app->params["privacy-policy"], ["target" => "_blank"]) ?>
                <?= " | " ?>
